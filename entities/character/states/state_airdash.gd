...
func can_start(character) -> bool:
	var axis = character.get_axis().round().normalized()
...
func on_update(delta, character, fsm):
	...
	if tick >= 0 and tick <= character.AIRDASH_LENGTH:

		# update airdash speed
		var airdash_speed = max(character.AIRDASH_SPEED, character.velocity.length())
		var t = pow(tick / float(character.AIRDASH_LENGTH), 2)
		character.velocity = (airdash_dir * lerpf(
			airdash_speed, character.AIRDASH_SPEED_END, t
		))

	# end of airdashing
	if tick > character.AIRDASH_LENGTH or character.is_grounded():
