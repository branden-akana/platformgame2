...
onready var model: Spatial = get_node("viewport_container/viewport/spatial/captain")
onready var model_player: AnimationPlayer = get_node("viewport_container/viewport/spatial/captain/animation_player")
...
func _process(_delta):
    ...
    if is_instance_valid(model):
        ...
        if fsm.current_state is RunningState or fsm.current_state is DashState:    
            model_player.current_animation = "running"
        elif fsm.current_state is IdleState:
            model_player.current_animation = "idle"
...
func play_animation(anim, from_end = false):
    if (
        is_instance_valid(model)
        and model_player.has_animation(anim)
        and model_player.current_animation != anim
    ):
        # 3D model
        model_player.play(anim, 0.01, 1.0, from_end)
        if anim == "attack":
            model_player.seek(0.5)
    else:
        # 2D sprite
...
func set_animation(anim):
    if is_instance_valid(model) and model_player.has_animation(anim):
        # 3D model
        model_player.current_animation = anim
    else:
        # 2D sprite
        $sprite.animation = anim
