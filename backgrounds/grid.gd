...
func _process(delta):
    if Game.has_method("get_camera"):
        position = origin + Game.get_camera().focus

    offset.x = fmod(offset.x + (delta * 10), 64)
    offset.y = fmod(offset.y + (delta * 10), 64)
    $box.texture_offset = ((Game.get_camera().focus * parallax) + offset).round()
    $box_2.texture_offset = (Game.get_camera().focus * parallax * 0.5).round()
